...
class CrudControllerCommand extends GeneratorCommand
{
    ...
    protected function getDefaultNamespace($rootNamespace)
    {
        ...
        return $rootNamespace . '\Modules\\'.$crudName.'\Http\Controllers';
    }
    ...
    protected function getPath($name)
    {
        ...
        $nome = array_pop($bl);
        $crudName = $this->option('crud-name');
        $path =  $this->rootNamespace()."\Modules\\$crudName\\Http\\Controllers\\$nome.php";
        return $path;
    }
    ...
    protected function buildClass($name)
    {
        ...
        return $this->replaceNamespace($stub, $name)
            ...
            ->replaceModelName($stub, $crudName)
    ...
    {
        $rt = trim(implode('\\', array_slice(explode('\\', $name), 0, -2)), '\\');
        $stub = str_replace(
            ['DummyNamespace', 'DummyRootNamespace'],
            [$rt, $this->rootNamespace()],
            $stub
        );
        return $this;
    }
    ...
    {
        $stub = str_replace(
            '{{modelName}}', ucfirst(str_singular($modelName)), $stub
        );

        $stub = str_replace(
            '{{moduleName}}', ucfirst($modelName), $stub
        );
